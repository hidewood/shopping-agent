...
### 3.1 语义程序（TurnProgram）

系统的核心设计是把自然语言**编译**为一个有序的语义程序。每轮模型输出一个 `TurnProgram`，根键固定为 `schema_version`、`primary_act`、`clauses`、`relations`：

- **`clauses`**：有序数组（≤8 个），18 种子句（15 种核心 + 3 种只读查询 `order_query / favorite_list / cart_query`）。每条子句对应一个动作，数组顺序即执行顺序——这使「一句话包含多个有依赖的动作」成为可表达、可校验的。
- **`relations`**：子句间依赖，仅 `before`（顺序）、`result_reference`（引用前序结果）、`conditional_non_empty / conditional_empty`（条件门控）。显式声明依赖，使「加购要引用比较的结果」这类关系可被校验。
- **`evidence`**：状态变更或账户动作子句携带用户原话的**精确子串**，作为可审计的证据。

这一设计把模型的职责限定在「语义编译」，把执行交给确定性代码。

### 3.2 证据约束（evidence）

`evidence` 是反幻觉的硬闸门：任何状态变更/账户动作子句必须携带用户原话的精确子串，否则直接判 `invalid_model_output`。这把「模型不能做用户没要求的事」固化为一条**硬校验规则**。

### 3.3 编译-执行分离

编译与执行严格分离：`_interpret`（模型 → TurnProgram）与 `_compile_and_execute`（TurnProgram → 结果 + 新聚合）是两个独立阶段，中间无模型参与。

以「比较 P0005 和 P0011，把便宜的加购物车」为例：

1. **编译**：DeepSeek 输出 `[product_query(比较), cart_add(加购)]`，`cart_add` 用 `result_reference` 指向 `product_query`。
2. **校验**：Python 校验每个子句 payload 是否合规、`evidence` 是否存在于原话。
3. **执行**：`product_query` 查目录得到两件商品 → `cart_add` 引用上一步结果，把便宜的那件编译成 `cart.add` effect。
4. **组装**：比较结果 + 加购 effect 一起返回。

### 3.4 确定性校验与接地

- **Schema 校验**：`_PAYLOAD_CONTRACTS` 用 JSON Schema 逐字校验每种子句 payload，未知字段直接拒绝，不合规触发一次修复。
- **确定性接地**：`_ground_constraints` 从用户原文 + 别名表 + 名称/描述做确定性匹配，不信任模型给的 `catalog_hint`；未知主题进入澄清，不做近似映射。
- **检索优先于追问**：只要类型已知就检索真实商品并报告如何收窄。

## 4. 状态执行

### 4.1 显式聚合（ConversationAggregate）

多轮状态由显式聚合对象 `ConversationAggregate` 承载——把选购状态**直接持有**在对象里，而非从事件日志重新推导。这体现「状态即数据」的原则：状态在任何时刻都是可直接读取的明确值，中断、重入、修改都一目了然。
...
- **澄清挂起**：`pending_clarification` + `pending_change_set` 在澄清时挂起未完成的变更。例如用户说「送人礼物」，系统问「马克杯还是T恤」，此时「礼物」这个需求挂在 `pending_change_set` 里，等用户回答类型后再一起重建；`interruption_policy="preserve_on_read_only"` 保证中间插入的只读查询不消费这个挂起的变更。
- **版本化快照**：推荐结果带 `plan_version` + `catalog_version`。目录一旦被管理员修改，旧快照立即失效，避免「推荐的商品已下架」这类错误。
...
系统的「理解」和「表达」是两端，中间隔着一层确定性执行。系统用两个提示词分别承担这两端：

- **编译前端（TURN_PROGRAM_SYSTEM）**：只让模型「理解」→ 输出结构化 `TurnProgram`。
- **接地渲染（GROUNDED_SUMMARY_SYSTEM）**：只让模型「表达」→ 把接地事实渲染成自然语言。
...
- **软引导（语义规则）**：把货币判定、`推荐N款` vs `买N件`、支付路由、送礼 recipient 等边界歧义写成显式规则。

这些规则从实测 bug 演化而来——每个 bug 修复后沉淀成一条规则，提示词因此是「踩坑踩出来的」。

### 6.3 两个 prompt 的内容

- **编译前端**：用 schema + 规则双重约束模型输出，把边界上的自由裁量权收回到确定性代码（货币判定、`推荐N款` vs `买N件`、支付→capability、送礼→recipient null、换一批→exclude_shown 等）。
- **接地渲染**：把接地后的事实（商品名、标签、价格）喂给模型生成自然语言，铁律是「只使用给定事实、绝不编造」，并区分 `selected`（主推荐）与 `options`（备选）。
