...
系统覆盖题目要求的需求理解、商品搜索、候选比较、约束检查、购买决策和结果说明，并实现多轮填槽、主动引导、状态隔离、无结果解释和交易能力边界。商品库包含 1,740 件商品，其中 `mug` 与 `shirt` 各 870 件。2026-08-10 的完整离线测试运行 114 项，其中 113 项通过、1 项真实 API 冒烟测试按默认配置跳过；历史真实 API 公开任务评测为 50/50 通过。本文不将历史 API 结果与本次离线测试混为同一批测量，而分别说明其证据范围。
...
3. 对少量高置信场景直接处理，例如无类型的礼物请求、显式开放式直接推荐、活动选购中的预算补充，以及“同轮购买 mug 和 shirt”的组合购买澄清；其余场景调用 DeepSeek 生成 `TurnPlan`；
...
| 目录查询/详情/比较 | `有什么价位`、`介绍 P0005` | 保留原选购状态 | 以只读方式执行，不覆盖活动 `ShoppingRequirement`；后续带明确筛选条件的选购可继续合并。 |
...
截至 2026-08-10，结果为 114 项运行、113 项通过、1 项真实 API 冒烟测试跳过。离线测试不依赖 API Key，重点检验目录真实性、状态迁移、路线正确性和近期修复不会回归。
...
| 证据层次 | 结果 | 可支持的结论 | 边界 |
| --- | --- | --- | --- |
| 当前离线套件 | 113/113 可执行测试通过；1 项真实 API 冒烟跳过 | 目录、状态、路由、价格区间、主动引导和近期修复的逻辑可回归验证。 | 不衡量真实 API 网络稳定性。 |
...
| 最新覆盖性人工回归 | 发现 2 条待修复路径：只读查询后的泛化续问、双类型并列价位查询 | 自动化基线通过不等同于穷尽所有自然语言变体；人工测试用于发现路由与呈现缺口。 | 两条问题将在修复后加入离线回归，不能计入当前已闭环能力。 |

### 6.1 对教师反馈的回应

**（1）意图识别精度。** 系统不把意图识别完全交给模型。强信号先约束比较、详情和交易路由，模型再在有限 `goal/target` 枚举中输出计划，之后由计划一致性和目录接地再次校验。早期人工审查中的“`Ocean主题`”问题表明，语言形式即使直观也可能在分词/映射中丢失；修复后将其直接接地为 Ocean 硬主题，并以 Trace 验证，而不是以“恰好推荐到 Ocean 商品”作为正确依据。
...
最新覆盖性人工测试还发现两条尚未闭环的路由缺口。其一，目录查询使用只读分支后，活动 `ShoppingRequirement` 的事件与 `TaskContext` 会被保留；但用户仅输入“给我推荐一个”时，当前实现尚未将该泛化续问确定性地转换为“复用现有条件的推荐”，可能重新询问商品类型。其二，“`mug 和 shirt 分别有什么价位？`”未被识别为组合购买，但当前单类型目录查询结构会将两个类型合并为一个无效类型，从而返回无法映射类别的提示。

这两项问题不涉及目录事实、硬约束校验或排序算法，而是发生在模型调用前后的续接路由与多类型只读查询建模。后续修复将增加“活动选购续接”和“多类型并列目录查询”两个受控分支，并补充中英文回归用例；在此之前，本文不把它们作为已完成能力。
...
6. 多类型的只读并列查询（例如分别询问 mug 与 shirt 的价位）尚未形成独立的数据结构；当前人工回归已定位该缺口，后续应以按类型并列统计替代单类型映射。
...
现有证据表明，项目的目录、状态、路由和核心约束逻辑具备离线回归基础，且历史真实 API 任务评测已记录为 50/50 通过。与此同时，最新人工回归定位出的“只读查询后的泛化推荐续接”和“双类型并列价位查询”尚未关闭，故不作为已实现能力计入结论。后续工作应先将这两条路径建模为受控分支并补齐中英文回归，再评估组合购买和真实订单能力的扩展。
