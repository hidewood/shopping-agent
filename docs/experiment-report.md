...
### 1.2 Failure of the Previous Design
...
## 5. Deterministic Execution and Transactions
...
## 9. Error Analysis：Where Does the Agent Fail?

### 9.1 Development 阶段发现并修复的 14 类缺陷
...
- **Internal validity**：开发集（121 条）参与了提示词与接地逻辑的调优，held-out（50 条）虽冻结，但二者由同一作者设计，可能存在措辞偏差。
- **External validity**：仅覆盖 `mug` / `shirt` 两类、1,740 件商品；结论不保证推广到更大、更多类型的目录。
- **Model dependency**：`TurnProgram` 的可靠性依赖 DeepSeek V4 Pro 的结构化输出能力；换模型可能改变边界失败率。
- **Dataset bias**：353 个标签中仅 44 个有中文别名（约 12%），中文模糊主题的覆盖有限，可能低估了接地失败率。
