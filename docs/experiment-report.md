...
系统覆盖题目要求的需求理解、商品搜索、候选比较、约束检查、购买决策和结果说明，并实现多轮填槽、主动引导、状态隔离、无结果解释和交易能力边界。商品库包含 1,740 件商品，其中 `mug` 与 `shirt` 各 870 件。2026-08-11 的完整离线测试运行 119 项，其中 118 项通过、1 项真实 API 冒烟测试按默认配置跳过；历史真实 API 公开任务评测为 50/50 通过。本文不将历史 API 结果与本次离线测试混为同一批测量，而分别说明其证据范围。
...
实现中，高置信分支的优先级也被固定：输入完整性、能力边界和无类型礼物请求在前；随后依次处理组合购买的待填槽续接、同轮组合购买识别、双类型只读价位查询，以及活动选购中的预算补充或泛化“推荐一个”续接；其余自由表达才进入模型规划。因此“`mug 和 shirt 分别有什么价位？`”不会误入组合购买，而短句“推荐一个”只有在已有活动选购条件时才会复用该条件。这些规则只覆盖低歧义表达，不替代模型对一般自然语言的理解。
...
- `ConversationState` 是会话容器，保存事件日志、待填槽问题、待填字段、最近只读查询上下文，以及组合购买的临时槽位；
...
| 同轮多类型购买 | `我想买马克杯和 T 恤，预算20以内` | `collecting` | 不将两类商品合并检索；先填入优先处理的类型，再确认预算是每件上限还是合计预算。两个槽位未完成时不执行单商品推荐；合计预算还须为当前商品给出单件上限。 |
...
图 3 只描述选购任务的主状态迁移。模型协议异常和服务超时属于图 2 中的失败分支：本轮返回 `service_error`，不以猜测结果改写选购需求；订单、支付和取消订单属于能力检查分支：返回 `capability_unavailable`，同样不产生交易状态。这样，选购状态机只承载可检索需求，异常处理和未实现能力不会混入推荐决策。
...
截至 2026-08-11，结果为 119 项运行、118 项通过、1 项真实 API 冒烟测试跳过。离线测试不依赖 API Key，重点检验目录真实性、状态迁移、路线正确性、组合购买填槽和近期修复不会回归。
...
| 证据层次 | 结果 | 可支持的结论 | 边界 |
| --- | --- | --- | --- |
| 当前离线套件 | 118/118 可执行测试通过；1 项真实 API 冒烟跳过 | 目录、状态、路由、价格区间、主动引导、组合购买填槽和近期修复的逻辑可回归验证。 | 不衡量真实 API 网络稳定性。 |
...
4. 当前组合购买可先后拆分为单商品子任务，但不执行自动合计预算分配、组合最优选择或真实订单创建；这些能力仍需独立的数据模型与执行器。
