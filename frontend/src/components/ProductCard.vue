...
<template>
  <div
    @click="handleClick"
    class="rounded-[26px] bg-surface overflow-hidden shadow-soft card-hover cursor-pointer border border-black/[0.03]"
  >
    <!-- Image (visual-first) -->
    <div class="aspect-[4/5] bg-gray-100 relative">
      ...
      <span v-if="variant === 'primary'" class="absolute top-3 left-3 text-[11px] bg-brand-500 text-white px-2.5 py-1 rounded-full font-medium">为你推荐</span>
    </div>

    <!-- Body -->
    <div class="p-4">
      <h3 class="font-semibold text-[14px] text-ink leading-snug line-clamp-1">{{ product.name }}</h3>
      <div class="text-[17px] font-bold text-ink mt-1">${{ product.price?.toFixed(2) }}</div>
