...
系统覆盖购物 Agent 的六项主干功能：

| 主干功能 | 实现方式 |
|---|---|
| 需求理解 | DeepSeek 语义编译（`TurnProgram`） |
| 商品搜索 | 目录接地 + 确定性检索 |
| 候选比较 | `product_query` 比较（如「比较 P0005 和 P0011」） |
| 约束检查 | 硬约束过滤 + 预算/厂商/主题检查 |
| 购买决策 | 确定性排序（偏好 → 厂商 → 标签 → 价格 → ID） |
| 结果说明 | 接地生成自然语言 + 个性化推荐理由 |
...
自动化测试代码和运行产物按项目约定只保留在开发者本地，不提交 GitHub。手动验收步骤见 [测试验收手册](docs/test-report.md)，代表性运行记录见 [运行记录](docs/run-records.md)，设计依据与可复现结论见 [实验报告](docs/experiment-report.md)。
